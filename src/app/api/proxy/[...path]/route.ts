...
const ALLOWED_POST_PATHS = [
  ...
  /^saved-payment-methods\/\d+\/set-default$/,
  /^saved-payment-methods\/\d+\/remove$/,
] as const;
...
function buildGetCacheKey(normalizedPath: string, request: NextRequest): string {
  ...
  const authorization = request.headers.get('authorization') || '';
  return `${normalizedPath}?${search}|auth:${authorization}`;
}
...
async function handleProxy(request: NextRequest, pathSegments: string[], method: 'GET' | 'POST'): Promise<NextResponse> {
  ...
  if (method === 'GET' && !isAllowedGetPath(normalizedPath)) {
    return jsonNoStore({ success: false, message: 'Not found' }, 404);
  }

  if (method === 'POST' && !isAllowedPostPath(normalizedPath)) {
  ...
  try {
    ...
    return new NextResponse(bodyText, {
      ...
      headers: {
        ...
        'Cache-Control': NO_STORE_CACHE_CONTROL,
        'X-Proxy-Cache': method === 'GET' ? 'MISS' : 'BYPASS',
      },
    });
  } catch (error) {
